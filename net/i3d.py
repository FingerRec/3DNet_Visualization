...
class InceptionI3d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self._spatial_squeeze:
            logits = x.squeeze(3).squeeze(3)  # remove dim whose size is 1
        logits = torch.mean(logits, 2)
        # print(logits)
        if self._num_classes != 400:
            logits_out = nn.Dropout(self.dropout_probality)(logits)
            fc_out = self.fc_out(logits_out)
            # print(fc_out.size()) # 4 x 101
            # print(self.softmax(fc_out)[0,:].data.cpu().numpy().sum())
            return fc_out
        #  return self.softmax(fc_out)
        else:
            # print(logits.size()) # 5 x101
...
def get_fine_tuning_parameters(model):
    ...
    # ft_module_names.append('attention_1')
