...
def parse_args():
    ...
    parser.add_argument("--supervised", type=str, default='fully_supervised',
                        choices=['fully_supervised', 'self_supervised'])
...
def heat_map_api(video, frames_num, clip_steps, output_dir, label, classes_list):
    ...
    for i in range(math.ceil((length - frames_num) // clip_steps)+1):
        ...
        if args.arch == 'mpi3d':
            ...
            if args.supervised == 'self_supervised':
        ...
        if args.arch == 'mpi3d':
            mask_img = visulaize.gen_mp_mask_img(RGB_vid[0][frames_num-1], heat_maps, pred_top3, prob_top3,
                                                 label,
                                                 classes_list)
        elif args.arch == 'i3d' and args.supervised == 'self_supervised':
            ...
            mask_img = visulaize.gen_mask_img(RGB_vid[0][frames_num -1], heat_maps, pred_top3, prob_top3,
                                              label, classes_list)
...
def main():
    ...
    for i in range(int(length/args.clip_steps) -1):
        ...
        for j in range(len(cam_list)):
            ...
            focus_imgs.append(focus_map)  # BGRA space
        if args.arch == 'mpi3d':
            mask_img = visulaize.gen_mp_mask_img(RGB_vid[0][args.frames_num//2], heat_maps, pred_top3, prob_top3, args.label,
                                              args.classes_list)
        elif args.arch == 'i3d' and args.supervised == 'self_supervised':
            mask_img = visulaize.gen_mask_img(RGB_vid[0][args.frames_num // 2], heat_maps, pred_top3, prob_top3,
                                              args.label, args.classes_list, text=False)
        else:
            mask_img = visulaize.gen_mask_img(RGB_vid[0][args.frames_num//2], heat_maps, pred_top3, prob_top3, args.label, args.classes_list)
        mask_imgs.append(mask_img)
